...
export default function PlanConfirmPage() {
  ...
  async function handleStartBooking() {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json() as { saved: boolean; reason?: string; id?: string }
        if (!data.saved && data.reason === 'unauthenticated') {
          console.warn('[itinerary/save] 미인증 사용자 — DB 저장 스킵, 로컬에만 저장')
        }
        setSaveStatus('saved')
