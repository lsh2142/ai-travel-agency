...
/**
 * Google OAuth PKCE 콜백 클라이언트 핸들러
 *
 * route.ts (서버)는 PKCE code_verifier에 접근할 수 없으므로
 * 이 클라이언트 컴포넌트가 브라우저의 Supabase 인스턴스를 통해
 * code → session 교환을 완료합니다.
 */
export default function CallbackClientPage() {
  ...
  useEffect(() => {
    ...
    const exchangeCode = async () => {
      try {
        // 브라우저 Supabase 클라이언트 — localStorage에서 code_verifier 자동 접근
